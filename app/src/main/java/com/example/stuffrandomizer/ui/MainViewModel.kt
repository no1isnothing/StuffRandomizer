...
class MainViewModel(private val matchRepository: MatchRepository) : ViewModel() {
    ...
    // start here, changing this to live data so it can be updated once it's loaded from the database
    ...
    fun getPreviewData() {
        viewModelScope.launch {
            ...
            println(matches[0].matchName)
            _previewString.value = matches[0].matchName// TODO change to livedata of some sort
        }
    }
